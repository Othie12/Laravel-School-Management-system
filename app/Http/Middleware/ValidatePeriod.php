    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        foreach($begin as $b){
            ...
            if($currentYear !== $b->year){
                return response()->json(['error' => 'You can only set calendar for this year.']);
            }
        }
        foreach($end as $e){
            $e = Carbon::parse($e);
        ...
        if($first_end->isBefore($first_begin) || $second_end->isBefore($second_begin) || $third_end->isBefore($third_begin)){
            return response()->json(['error' => 'You have set start date greater than the end date.']);
        }elseif($first_end->isAfter($second_begin) || $second_end->isAfter($third_begin)){
            return response()->json(['error' => 'Your dates are not realistic.']);
